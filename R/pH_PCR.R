...
#'
#' This function compares pH data against water quality standards and/or thresholds for
#' primary contact recreation (PCR) use. For more information, see the \href{https://www.env.nm.gov/surface-water-quality/calm/}{NMED Consolidated Assessment and Listing Methodology (CALM)} document.
#'
...
pH_PCR <- function(Chem_table
  ...
  # pH PCR ####
  RFunctionName <- "pH_PCR"
  DU_Name <- "PC"

  # Only relevant criteria in DU table
  df_DU_v2 <- DU_table %>%
    dplyr::filter(DU == DU_Name) %>%
    dplyr::filter(CHR_UID == "1648") %>% #PH_HIGH and PH_LOW
    dplyr::select(-c(TN_SITE_CLASS, TP_SITE_CLASS)) %>%
    dplyr::distinct()

  (CHR_UID_pH_PCR <- unique(df_DU_v2$CHR_UID))

  ## Trim chem data ####
  df_Chem_v2 <- Chem_table %>%
    dplyr::filter(CHR_UID %in% CHR_UID_pH_PCR)
    ...
    for(j in my_constituents) {
      ...
      if(n_Samples == 1){
        results <- df_subset_v3 %>%
          ...
          dplyr::mutate(bad_samp = ifelse(CHR_UID_Unique == "1648b"
  ...
  df_pH_PCR <- as.data.frame(do.call("rbind", result_list))
