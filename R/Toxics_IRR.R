...
#' standards and/or thresholds for irrigation (IRR) use. For more information, see the
...
#'
#' @returns A dataframe containing analyzed toxics data compared to IRR
...
Toxics_IRR <- function(Chem_table
  ...
  CHR_UID_ToxicIRR <- unique(df_Crit_v2$CHR_UID)

  ## Trim chem data ####
  df_Chem_v2 <- Chem_table %>%
    dplyr::filter(CHR_UID %in% CHR_UID_ToxicIRR)
  ...
  for(i in Unique_AUIDs){
    ...
    for(j in my_constituents){
      ...
      # Apply method based on Irrigation criteria
      Crit_IRR <- df_Crit_v3 %>%
      ...
      if(n_Samples == 1){
        results <- df_subset_v2 %>%
          dplyr::mutate(bad_samp = dplyr::case_when((LESS_THAN_YN == "Y"
                                                     & (Crit_IRR <= MEASUREMENT_num))  ~ 0
                                                    , MEASUREMENT_num > Crit_IRR ~ 1
                                                    , TRUE ~ 0),
                        FLAG_MRLgrtrCrit = dplyr::case_when((LESS_THAN_YN == "Y"
                                                             & (Crit_IRR <= MEASUREMENT_num))
                                                            ~ "Nonassessable"
                                                            , TRUE ~ NA))
  ...
  df_Toxics_IRR <- as.data.frame(do.call("rbind", result_list))

  # Export data ####
  return(df_Toxics_IRR)
