...
#' standards and/or thresholds for wildlife habitat (WH) use. For more information, see the
...
#'
#' @returns A dataframe containing analyzed toxics data compared to WH
...
Toxics_WH <- function(Chem_table
  ...
  CHR_UID_ToxicWH <- unique(df_Crit_v2$CHR_UID)

  ## Trim chem data ####
  df_Chem_v2 <- Chem_table %>%
    dplyr::filter(CHR_UID %in% CHR_UID_ToxicWH)
  ...
  for(i in Unique_AUIDs){
    ...
    for(j in my_constituents){
      ...
      # Apply method based on Wildlife Habitat criteria
      Crit_WH <- df_Crit_v3 %>%
      ...
      if(n_Samples == 1){
        results <- df_subset_v2 %>%
          dplyr::mutate(bad_samp = dplyr::case_when((LESS_THAN_YN == "Y"
                                                     & (Crit_WH <= MEASUREMENT_num))  ~ 0
                                                    , MEASUREMENT_num > Crit_WH ~ 1
                                                    , TRUE ~ 0),
                        FLAG_MRLgrtrCrit = dplyr::case_when((LESS_THAN_YN == "Y"
                                                             & (Crit_WH <= MEASUREMENT_num))
                                                            ~ "Nonassessable"
                                                            , TRUE ~ NA))
  ...
  df_Toxics_WH <- as.data.frame(do.call("rbind", result_list))
