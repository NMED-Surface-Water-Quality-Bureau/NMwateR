...
#'
#' This function compares toxics data against water quality standards and/or thresholds for
#' drinking water supply (DWS) use. For more information, see the \href{https://www.env.nm.gov/surface-water-quality/calm/}{NMED Consolidated Assessment and Listing Methodology (CALM)} document.
#'
...
#'
#' @returns A dataframe containing analyzed toxics data compared to DWS
...
Toxics_DWS <- function(Chem_table
  ...
  CHR_UID_ToxicDWS <- unique(df_Crit_v2$CHR_UID)

  ## Trim chem data ####
  df_Chem_v2 <- Chem_table %>%
    dplyr::filter(CHR_UID %in% CHR_UID_ToxicDWS)
  ...
  for(i in Unique_AUIDs){
    ...
    for(j in my_constituents){
      ...
      # Apply method based on Domestic Water Supply criteria
      Crit_DWS <- df_Crit_v3 %>%
      ...
      if(n_Samples == 1){
        results <- df_subset_v2 %>%
          dplyr::mutate(bad_samp = dplyr::case_when((LESS_THAN_YN == "Y"
                                                     & (Crit_DWS <= MEASUREMENT_num))  ~ 0
                                                    , MEASUREMENT_num > Crit_DWS ~ 1
                                                    , TRUE ~ 0),
                        FLAG_MRLgrtrCrit = dplyr::case_when((LESS_THAN_YN == "Y"
                                                             & (Crit_DWS <= MEASUREMENT_num))
                                                            ~ "Nonassessable"
                                                            , TRUE ~ NA))
  ...
  df_Toxics_DWS <- as.data.frame(do.call("rbind", result_list))

  # Export data ####
  return(df_Toxics_DWS)
