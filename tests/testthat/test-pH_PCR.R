test_that("pH PCR calc works", {
  ...
  n_row_code <- nrow(df_pH_PCR)
  sum_n_Samps_code <- sum(df_pH_PCR$n_Samples, na.rm = TRUE)
  sum_n_Exceed_code <- sum(df_pH_PCR$n_Exceed, na.rm = TRUE)
