...
fn.data7 <- "NMED_Chem_Processed_20251204.csv"
fn.data8 <- "NMED_DU_Processed_20251204.csv"
fn.data9 <- "NMED_WQ_Criteria_Formatted_20251204.csv"
...
example_chemistry_processed <- readr::read_csv(file.path(wd, input.dir, fn.data7)
                                               , na = c("NA","")
                                               , trim_ws = TRUE, skip = 0
                                               , col_names = TRUE, guess_max = 100000)

example_DU_processed <- readr::read_csv(file.path(wd, input.dir, fn.data8)
                                        , na = c("NA","")
                                        , trim_ws = TRUE, skip = 0
                                        , col_names = TRUE, guess_max = 100000)

example_criteria_processed <- readr::read_csv(file.path(wd, input.dir, fn.data9)
                                              , na = c("NA","")
                                              , trim_ws = TRUE, skip = 0
                                              , col_names = TRUE, guess_max = 100000)
